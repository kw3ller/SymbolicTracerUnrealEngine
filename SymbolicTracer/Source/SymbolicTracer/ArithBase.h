...
#include "ArithBase.generated.h"

class UInstrStepsListBase;
class ARTSPlayerPawnST;
class UTextBlock;
/**
 * Widget for showing details of arithmetic instructions
 */
UCLASS(Abstract)
class SYMBOLICTRACER_API UArithBase : public UWidgetSTBase
{
	GENERATED_BODY()
...
protected:
	...
	UPROPERTY(BlueprintReadOnly, Category="Clicked Indices", meta=(BindWidget))
	TObjectPtr<UTextBlock> Rs2Text = nullptr;

	UPROPERTY(BlueprintReadOnly, Category="Clicked Indices", meta=(BindWidget))
	TObjectPtr<UTextBlock> RdText = nullptr;

	UPROPERTY(BlueprintReadOnly, Category="Clicked Indices", meta=(BindWidget))
	TObjectPtr<UTextBlock> Imm1Text = nullptr;
	...
	UPROPERTY()
	FText CurrentRs2Text = FText::FromString("Rs2: -1");

	UPROPERTY()
	FText CurrentRdText = FText::FromString("Rd: -1");

	UPROPERTY()
	FText CurrentImm1Text = FText::FromString("Imm1: -1");
